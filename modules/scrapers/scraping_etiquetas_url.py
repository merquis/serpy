...
def render_scraping_etiquetas_url():
    ...
    if "scraping_resultado" not in st.session_state:
        st.session_state["scraping_resultado"] = None
    if "scraping_nombre_archivo" not in st.session_state:
        st.session_state["scraping_nombre_archivo"] = None
    ...
    if fuente == "Desde ordenador":
        ...
        if cargar:
            ...
            st.session_state["json_contenido"] = contenido
            ...
            st.session_state["scraping_resultado"] = salida
            nombre_base = archivo_drive
            nombre_final = nombre_base.replace(".json", "_ALL.json") if nombre_base.endswith(".json") else nombre_base + "_ALL.json"
            st.session_state["scraping_nombre_archivo"] = nombre_final

    # ───── Mostrar resultado si ya existe ─────
    if st.session_state["scraping_resultado"]:
        st.markdown("### 💾 Exportar resultado")
        nombre_archivo = st.text_input(
            "📄 Nombre para exportar el archivo JSON",
            value=st.session_state["scraping_nombre_archivo"]
        )
        ...
        with col_export[0]:
            st.download_button(
                ...
                data=json.dumps(st.session_state["scraping_resultado"], ensure_ascii=False, indent=2),
                file_name=nombre_archivo,
                mime="application/json"
            )
        with col_export[1]:
            if st.button("☁️ Subir archivo a Google Drive"):
                contenido_bytes = json.dumps(st.session_state["scraping_resultado"], ensure_ascii=False, indent=2).encode("utf-8")
        ...
        st.json(st.session_state["scraping_resultado"])
