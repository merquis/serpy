...
# ════════════════════════════════════════════════════
# 🔎 Verificar IP pública (con proxy)
# ════════════════════════════════════════════════════
async def verificar_ip(page):
    try:
        ...
        st.session_state["ip_con_proxy"] = "error"
...
async def obtener_datos_booking_playwright(url: str, browser_instance=None):
    ...
    try:
        ...
        if st.session_state.get("ip_real") == st.session_state.get("ip_con_proxy"):
            print("🚫 Proxy NO funcionando. Abortando scraping.")
            return {
                "error": "Proxy no funcionando",
                "ip_real": st.session_state.get("ip_real", "desconocida"),
                "ip_con_proxy": st.session_state.get("ip_con_proxy", "desconocida"),
                "url": url
            }, ""
...
def render_scraping_booking():
    ...
    st.title("🏨 Scraping hoteles Booking (BrightData + Proxy)")

    if "urls_input" not in st.session_state:
        st.session_state.urls_input = "https://www.booking.com/hotel/es/barcelo-tenerife-royal-level.es.html?checkin=2025-05-12&checkout=2025-05-13&group_adults=2&group_children=0&no_rooms=1&dest_id=-369166&dest_type=city"
